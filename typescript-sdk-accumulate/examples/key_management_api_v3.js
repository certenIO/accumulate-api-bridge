...
import { api_v2, api_v3, ED25519Key, Signer } from "accumulate.js";
...
async function keyManagementExample() {
    ...
    console.log(`Initial balance: ${balanceQuery.account.balance} ACME tokens\n`);
    ...
    try {
        ...
        console.log(`Key page exists: ${keyPageCheck.account.type}`);
    }
    catch (error) {
    ...
    for (let i = 0; i < 10; i++) {
        try {
            ...
            const credits = keyPageQuery.account?.creditBalance || 0;
    ...
    const currentPageCount = bookQuery.account.pageCount || 1;
    ...
    try {
        ...
        console.log(`New page type: ${newPageQuery.account.type}\n`);
    }
    catch (error) {
    ...
    try {
        ...
        console.log(`Book2 type: ${book2Query.account.type}`);
        console.log(`Book2 page count: ${book2Query.account.pageCount}\n`);
    }
    catch (error) {
    ...
    try {
        ...
        const keys = updatedPageQuery.account.keys || [];
    ...
    try {
        ...
        currentKeys = currentPageQuery.account.keys || [];
    ...
    try {
        const updatedPageQuery = await client.query(keyPageUrl);
        currentKeys = updatedPageQuery.account.keys || [];
    ...
    try {
        ...
        const finalKeys = finalPageQuery.account.keys || [];
...
//# sourceMappingURL=key_management_api_v3.js.map