...
export function fieldMarshalBinary(field, val) {
    if (field < 1 || field > 32) {
        throw new Error(`Field number is out of range [1, 32]: ${field}`);
    }
    return Buffer.concat([uvarintMarshalBinary(field), val]);
}
export function uvarintMarshalBinary(val, field) {
    if (typeof val === "number" && val > Number.MAX_SAFE_INTEGER) {
        throw new Error("Cannot marshal binary number greater than MAX_SAFE_INTEGER. Use bigint instead.");
    }
    let x = BigInt(val);
    const buffer = [];
    let i = 0;
...
export function varintMarshalBinary(val, field) {
    if (typeof val === "number") {
        if (val > Number.MAX_SAFE_INTEGER) {
            throw new Error("Cannot marshal binary number greater than MAX_SAFE_INTEGER. Use bigint instead.");
        }
        if (val < Number.MIN_SAFE_INTEGER) {
            throw new Error("Cannot marshal binary number less than MIN_SAFE_INTEGER. Use bigint instead.");
        }
    }
...
export function bigNumberMarshalBinary(bn, field) {
    // @ts-ignore
    if (bn < 0n) {
...
export function booleanMarshalBinary(b, field) {
    const data = b ? Uint8Array.from([1]) : Uint8Array.from([0]);
    return withFieldNumber(data, field);
}
export function stringMarshalBinary(val, field) {
    const data = bytesMarshalBinary(Buffer.from(val));
    return withFieldNumber(data, field);
}
export function bytesMarshalBinary(val, field) {
    if (!val)
        val = new Uint8Array();
...
export function hashMarshalBinary(val, field) {
    if (val.length != 32) {
