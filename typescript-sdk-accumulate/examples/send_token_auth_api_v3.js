...
async function sendTokenAuthExample() {
    ...
    console.log(`Initial LTA balance: ${balanceQuery.account.balance} ACME tokens\n`);
    ...
    console.log(`LID credits balance: ${creditsQuery.account?.creditBalance || 0} credits\n`);
    ...
    console.log(`ADI Identity type: ${identityQuery.account.type}\n`);
    ...
    for (let i = 0; i < 5; i++) {
        try {
            ...
            const credits = keyPageQuery.data?.creditBalance || 0;
    ...
    for (let i = 0; i < 5; i++) {
        try {
            ...
            console.log(`Token Account type: ${tokenAccountQuery.account.type}`);
            console.log(`Token Account balance: ${tokenAccountQuery.account.balance || 0} ACME\n`);
            break;
        }
        catch (error) {
            if (i === 4)
    ...
    let tokenBalanceAfter1;
    for (let i = 0; i < 20; i++) {
        try {
            await new Promise(resolve => setTimeout(resolve, 3000)); // Wait 3 seconds between checks
            tokenBalanceAfter1 = await client.query(tokenAccountUrl);
            const balance = tokenBalanceAfter1.account.balance || 0;
    ...
    console.log(`LTA balance after send: ${ltaBalanceAfter1.account.balance} ACME`);
    console.log(`Token Account balance after receive: ${tokenBalanceAfter1.account.balance || 0} ACME\n`);
    ...
    console.log(`LTA final balance: ${ltaBalanceFinal.account.balance} ACME`);
    console.log(`Token Account final balance: ${tokenBalanceFinal.account.balance} ACME\n`);
    ...
    console.log(`Current token account authorities:`, currentTokenAccount.account.authorities || []);
    ...
    try {
        ...
        console.log(`Account status after disable:`, disabledAccount.account);
    }
    catch (error) {
    ...
    try {
        ...
        console.log(`Final authorities:`, finalTokenAccount.account.authorities || []);
        console.log(`Final balance: ${finalTokenAccount.account.balance} ACME`);
    }
    catch (error) {
...
async function getCurrentKeyPageVersion(keyPageUrl) {
    try {
        ...
        const version = keyPageQuery.data?.version;
...
async function waitForSingle(txid) {
    ...
    for (let i = 0; i < waitLimit; i++) {
        try {
            ...
            if (status && (status === Status.Delivered || status === 201 || status.code === "delivered")) {
...
//# sourceMappingURL=send_token_auth_api_v3.js.map