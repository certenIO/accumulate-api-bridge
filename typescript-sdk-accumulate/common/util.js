...
import { combineReceipts, Receipt } from "../merkle/index.js";
import { Buffer } from "./buffer.js";
import { sha256 } from "./sha256.js";
export async function constructIssuerProof(client, issuer) {
    ...
    if (transaction.body.type != "createToken") {
        throw new Error(`Expected first transaction of ${issuer} to be createToken but got ${transaction.body.type}`);
    }
    ...
    const receipt = combineReceipts(combineReceipts(new Receipt(proof1), new Receipt(proof2)), new Receipt(proof3));
