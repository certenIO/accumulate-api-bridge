...
import { LedgerApi, promise, queryHidWallets, registerTransportModule, } from "accumulate.js/ledger";
...
registerTransportModule({
    id: "hid",
    open: (devicePath) => promise.retry(() => TransportNodeHid.open(devicePath), {
        context: "open-hid",
    }),
    discovery: new Observable(TransportNodeHid.listen).pipe(map((e) => ({
        type: e.type,
        id: e.device.path,
...
(async () => {
    ...
    if (rest.length)
        throw new Error("multiple connected wallets");
    // Open it
